...
from app.approval.repository import count_pending_approvals, create_approval, expire_pending_approvals, get_approval, mark_rejected
...
class ApprovalService:
    ...
    def cleanup_expired(self) -> int:
        return expire_pending_approvals(self._settings)
    ...
    def create_pending(
        ...
        *,
        ...
        if count_pending_approvals(self._settings) >= self._settings.approval_max_pending:
            raise ApprovalError(409, "APPROVAL_NOT_PENDING", "Pending approval limiti oshdi.")
    ...
    def get_public(self, approval_id: str) -> object:
        expire_pending_approvals(self._settings)
    ...
    def validate_nonce_and_origin(self, approval_id: str, nonce: str, origin_or_referer: str | None) -> object:
        ensure_approvals_enabled(self._settings)
        expire_pending_approvals(self._settings)
        approval = get_approval(self._settings, approval_id)
        if approval is None:
    ...
    def reject(self, approval_id: str, nonce: str, origin_or_referer: str | None) -> object:
        approval = self.validate_nonce_and_origin(approval_id, nonce, origin_or_referer)
