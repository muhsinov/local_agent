...
def test_document_service_create_and_get(tmp_path) -> None:
    ...
    with TestClient(app):
        document = create_document(
            settings,
            file_name="note.txt",
            file_path="data/uploads/a.txt",
            file_type="txt",
            size_bytes=5,
            sha256="abc",
            status="ready",
            text_path="data/extracted/a.txt",
            char_count=5,
            page_count=None,
            warning_code=None,
        )
...
def test_document_service_list_orders_desc(tmp_path) -> None:
    ...
    with TestClient(app):
        create_document(
            settings,
            file_name="one.txt",
            file_path="data/uploads/one.txt",
            file_type="txt",
            size_bytes=1,
            sha256="1",
            status="ready",
            text_path="data/extracted/one.txt",
            char_count=1,
            page_count=None,
            warning_code=None,
        )
        create_document(
            settings,
            file_name="two.txt",
            file_path="data/uploads/two.txt",
            file_type="txt",
            size_bytes=2,
            sha256="2",
            status="ready",
            text_path="data/extracted/two.txt",
            char_count=2,
            page_count=None,
            warning_code=None,
        )
